...
const  ClubAffiliationList= ()=> {
    ...
    const getData = ({cin:cin,page:page}) => {
        AdminServices.getClubsAffiliations({type: "active",cin:cin,page:page}).then(response => {
    ...
    return (
        <>
            ...
            <Table pagination={{total: apiResponse.total, defaultPageSize: 10,
                ...
                }}} scroll={{x: 1024}} size={'small'} showHeader columns={column} data={data} dataSource={data} />
            <Modal
                title={'Detail Club'}
                centered
                visible={visible}
